...
class RESPARSE:
	...
	def parselist(self, res, url):
		...
		try:
			...
			next_url = urljoin(url, self.html.xpath("//div[@class='profile container']/div[4]/div[2]/p/a[3]/@href")[0])
			code_urls = []
			if(len(addresss)):
				for address in addresss:
					...
					_url = urljoin(url, _xml_url[0]) if len(_xml_url)>0 else ""
					_ContractName = _xml_contractname[0] if len(_xml_contractname)>0 else ""
					_DateVerified = Date2path(_xml_dateVerified[0]) if len(_xml_dateVerified)>0 else ""
					code_urls.append(_url+',' + _ContractName+','+ _DateVerified+'\n')
	...
	def parsecode(self, res, url):
		...
		_CSCO=None;_CSC=None;_CCC = None
		if len(_pres) >= 2:
	...
	def parse_list_page_count(self, res):
		self.run(res)
		_counts = self.html.xpath("//div[@class='profile container']/div[2]/div[2]/p/span/b[2]/text()")
		counts = []
		if _counts:
			counts = _counts[0]
		return int(counts)
	def load_arm_code(self):
		...
		api_url = "https://rinkeby.etherscan.io/api?module=opcode&action=getopcode&address={addr}".format(addr=address)
